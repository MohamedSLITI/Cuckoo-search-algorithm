...
class BaseVisualizer(VisualizerBase):
    ...
    def _animate(self, i: int, frames: int):
        ...
        self._fig.canvas.set_window_title(
            f'Iteration {np.minimum(self._index, self.__iteration_number)}/{self.__iteration_number}')
        ...
        self.__particle_vel = ax.quiver(pos_scaled[0], pos_scaled[1], vel_scaled[0], vel_scaled[1], angles='xy',
                                        scale_units='xy', scale=1, color=self._vel_color,
                                        width=self._marker_size * 0.001)
