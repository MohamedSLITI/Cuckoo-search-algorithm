...
class CuckooProblem(ProblemBase):
    ...
    def solve(self) -> Nest:
        ...
        logging.info('Iteration 0 best solution="%s" at position="%s"', best_nest.value, best_nest.position)
        for iteration in range(self.__max_generations):

            # Perform levy flights to get cuckoo's new position
            new_cuckoo_pos = [
                np.clip(cuckoo.levy_flight(nest.position, self.__alpha, self.__lambda, self._random),
                        a_min=self.__lower_boundary, a_max=self.__upper_boundary)
            ...
            if current_best < best_nest:
                ...
                LOGGER.info('Iteration %i Found new best solution="%s" at position="%s"', iteration + 1,
                            best_nest.value, best_nest.position)

            # Add data for plot
            positions, abandoned = zip(*[(nest.position, nest.abandoned) for nest in self.__nests])
            self._visualizer.add_data(positions=positions, best_position=current_best.position, abandoned=abandoned)

        logging.info('Last best solution="%s" at position="%s"', best_nest.value, best_nest.position)
